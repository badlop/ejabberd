...
mod_options(Host) ->
    [{db_type, ejabberd_config:default_db(Host, ?MODULE)},
     {room_tombstone_expiry, infinity},
     {user_tombstone_expiry, infinity}].

mod_doc() ->
    #{desc =>
          [?T("Keep tombstones for accounts and rooms that were removed. "
              "Prevent registration of that account, and creation of that room.")],
      note => "added in 24.xx",
      opts =>
          [{db_type,
           ...
           {room_tombstone_expiry,
            #{value => ?T("time | infinity"),
              desc =>
                  ?T("How long to keep MUC room tombstones, for example '30 days'. "
                     "The default value is 'infinity', keep tombstones forever.")}},
