...
route(Pkt, ServerHost) ->
    ...
    To = xmpp:get_to(Pkt),
    Host = To#jid.lserver,
    Access = get_option(ServerHost, To#jid.lserver, access),
...
route(#message{lang = Lang, body = Body, type = Type, from = From,
    ...
    if Type == error ->
            ok;
       true ->
	    AccessAdmin = get_option(ServerHost, Host, access_admin),
...
handle_call({create, Room, Host, Opts}, _From,
    ...
    NewOpts = case Opts of
		  default -> get_option(ServerHost, Host, default_room_options);
		  _ -> Opts
	      end,
    RMod = gen_mod:ram_db_mod(ServerHost, ?MODULE),
...
process_vcard(#iq{type = get, to = To, lang = Lang, sub_els = [#vcard_temp{}]} = IQ) ->
    ServerHost = ejabberd_router:host_of_route(To#jid.lserver),
    VCard = case get_option(ServerHost, To#jid.lserver, vcard) of
		undefined ->
...
process_iq_register(#iq{type = Type, from = From, to = To, lang = Lang,
    ...
    ServerHost = ejabberd_router:host_of_route(Host),
    AccessRegister = get_option(ServerHost, Host, access_register),
...
process_disco_info(#iq{type = get, from = From, to = To, lang = Lang,
    ...
    ServerHost = ejabberd_router:host_of_route(To#jid.lserver),
    RMod = gen_mod:ram_db_mod(ServerHost, ?MODULE),
    AccessRegister = get_option(ServerHost, To#jid.lserver, access_register),
    ...
    Name = get_option(ServerHost, To#jid.lserver, name),
...
process_disco_items(#iq{type = get, from = From, to = To, lang = Lang,
    ...
    Host = To#jid.lserver,
    ServerHost = ejabberd_router:host_of_route(Host),
    MaxRoomsDiscoItems = get_option(ServerHost, Host, max_rooms_discoitems),
...
check_create_room(ServerHost, Host, Room, From) ->
    AccessCreate = get_option(ServerHost, Host, access_create),
    case acl:match_rule(ServerHost, AccessCreate, From) of
	allow ->
	    case get_option(ServerHost, Host, max_room_id) of
		Max when byte_size(Room) =< Max ->
		    Regexp = get_option(ServerHost, Host, regexp_room_id),
...
get_access(ServerHost, Host) ->
    Access = get_option(ServerHost, Host, access),
    AccessCreate = get_option(ServerHost, Host, access_create),
    AccessAdmin = get_option(ServerHost, Host, access_admin),
    AccessPersistent = get_option(ServerHost, Host, access_persistent),
    AccessMam = get_option(ServerHost, Host, access_mam),
...
start_new_room(RMod, Host, ServerHost, Room, Pass, From, Nick) ->
    ...
    DefRoomOpts = get_option(ServerHost, Host, default_room_options),
...
start_room(Mod, Host, ServerHost, Room, DefOpts) ->
    ...
    HistorySize = get_option(ServerHost, Host, history_size),
    QueueType = get_option(ServerHost, Host, queue_type),
    RoomShaper = get_option(ServerHost, Host, room_shaper),
...
-spec get_option(binary(), binary(), atom()) -> any().
get_option(ServerHost, MucHost, Option) ->
    Appends = mod_muc_opt:append_module_config(ServerHost),
    GlobalValue = mod_muc_opt:Option(ServerHost),
    case proplists:get_value(MucHost, Appends) of
        undefined ->
            GlobalValue;
...
%% Code copied from ejabberd_options.erl
            ...
            #{value => "{MUCHost: Options}",
              ...
              example =>
                  ["modules:",
                   ...
                   "    append_module_config:",
                   "      chat.localhost:",
                   "        name: \"Local chat rooms\"",
                   "      chat.example.net:",
                   "        name: \"Public chat rooms\"",
                   "      muc.localhost:",
                   "        name: \"Local MUC rooms\""]}},
           {db_type,
            #{value => "mnesia | sql",
              desc =>
